...
|---|---------|------------------|--------|------------|
| 1 | **DARION-AI · Multi-Tenant-SaaS** | Modulare Business-Suite mit 24 Fachmodulen, davon 21 verfügbar (u. a. Finanzen, Buchhaltung, HR, Zeiterfassung, Vertrieb, Marketing, Projekte/PSA, Verträge, E-Rechnung, Einkauf, Lager, Shop, IT-Management, Tickets, DMS, Wissen, ISMS, DSGVO-Compliance, Berichte), 3 im Ausbau (Meetings, Kalender, Works); 14 Tage kostenlos testbar, Mandant in Minuten angelegt; Multi-LLM-Routing, RAG-Layer, Live-Cockpit, CNPG-HA mit Pooler, CI/CD ≥ 80 % Coverage | GA 03.09.2026 | FastAPI · Next.js 15 · K3s · CloudNativePG · RAG · OpenTelemetry |
...
| 18 | **KI-Schulung und Nachweis · EU AI Act Art. 4** | Schulungs- und Nachweismodul für KI-Kompetenz (AI-Literacy-Pflicht seit 02/2025): drei rollenbasierte Lernpfade (Basis, Management, Power-User) mit Test, automatischem Zertifikat und prüfsicherem Nachweis je Mitarbeiter in der HR-Ansicht; USP ist der integrierte, revisionssichere Nachweis (Hash-Chain-Audit-Log). Backend vollständig gebaut (Schema, RLS, SECURITY-DEFINER-RPCs, Hash-Chain), Curriculum DE/NL mit 30-Fragen-Test | In Entwicklung | PostgreSQL · Hash-Chain-Audit · WeasyPrint · Entitlements · EU AI Act Art. 4 |
...
| 20 | **DARION-AI · installierbare PWA mit Web-Push** | Business-Suite als installierbare Progressive Web App (mobiler Zugriff, Offline-Route) mit In-App-Web-Push für Echtzeit-Benachrichtigungen ohne zusätzliche App; Push-Subscriptions je Mandant unter RLS | Live | Next.js PWA · Service Worker · Web-Manifest · VAPID/web-push · Supabase RLS |
| 21 | **DARION-AI · Marketing- und Newsletter-Automation** | Nativer Versand von Newslettern/Kampagnen aus der Plattform: Double-Opt-in, Drip-Sequenzen, Lead-Scoring, RFC-8058-One-Click-Abmeldung, eigene Suppression-Pipeline über eigenen Mailserver (kein Fremd-Subprozessor) | Live | Next.js · PostgreSQL RLS · Mailcow (DKIM/VERP) · Postfix · postal-mime |
| 22 | **DARION-AI · Buchhaltung (vorbereitend) und DATEV-Export** | Beleg-Import (ZUGFeRD-Parser), Buchungserfassung, GoBD-konforme Festschreibung und DATEV-EXTF-Export mit Freigabe-Workflow; Modul gebaut und mit grüner Test-Suite, freigeschaltet | Live | Next.js · PostgreSQL RLS · ZUGFeRD · GoBD-Festschreibung · DATEV-EXTF |
| 23 | **DARION-AI · Berichte (Reporting) — Dashboards und Auswertungen** | Auswertungsmodul mit Dashboards, Reports und Kennzahlen über echte Aggregate aus Finanzen/Projekte/ISMS, planbaren Report-Runs und Export; Modul gebaut und getestet, freigeschaltet | Live | Next.js · PostgreSQL RLS · Aggregations-Services · Report-Runs |
| 24 | **Control4you · Plattform für verteilte Wasseraufbereitungscontainer** | Zentrale Steuer- und Überwachungsplattform: die Zentrale beantragt, der Container entscheidet, Safety bleibt in der SPS. Serverseitiger Kommandokatalog mit Risikoklassen, Vier-Augen-Freigabe, Passkey-Pflicht mit Step-up, FORCE RLS, Geräte-Gateway nur im Mesh mit mTLS, 13-stufige Sicherheitskette mit versiegelten Belegen, Totmannschalter je Automat, CRA-Meldeweg | Fundament und Slices 1–2 gebaut | Go · NATS · PostgreSQL RLS · Keycloak/Passkeys · mTLS · NetBird · Forgejo Actions · trivy/gosec/govulncheck · CycloneDX |
| 25 | **Clentaro · Versorgungsplattform für MVZ und Praxen** | Produktthese CareLoop („Wer hat gerade den Ball?"): Übergaben, Verantwortlichkeiten und Abschlüsse als Strom; über 130 FORCE-RLS-Policies, dedizierter Datenbank-Cluster je Kunde, GDT-Anbindung; Lieferungen unabhängig mit Messungen verifiziert | In Entwicklung | Kotlin · PostgreSQL RLS · Flyway · Keycloak · GDT · NetBird |
| 26 | **Aquakultur-KI · Kamera- und Sensor-Intelligenz für Kreislaufanlagen** | Ereignis-Stack mit Frigate und MQTT, Über-Wasser-Kamera zur Zählung und Längenmessung, lizenzsichere Detektoren und eigene Modelle mit Messbank; zentrale KI nur beobachtend und empfehlend, Sicherheit über harte Interlocks ohne Rechner; Rechtsrahmen recherchiert | Konzept, Entscheidungen getroffen | Frigate · MQTT · ONVIF Profile T · RT-DETR/RF-DETR · VictoriaMetrics · CloudNativePG · Chronos-2 |
| 27 | **Betriebs-Wächter · Totmannschalter für jeden Automaten** | Bauprinzip für Skripte, Cronjobs und Dienste (sicher, nachhaltig, modular, flexibel, gemonitored, reviewt): Zeitstempel je Automat, Wächter alarmiert bei Überalterung über Push und E2EE-Matrix mit Entwarnung; Flotten-Audit deckte stille Ausfälle hinter grünen Erfolgslogs auf | In Betrieb | healthchecks · ntfy · Matrix E2EE · Bash · cron/systemd · Prometheus |
